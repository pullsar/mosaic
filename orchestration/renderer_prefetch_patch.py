...
/// Warms only a bounded, current feed window.
///
/// Starting a new window supersedes queued work from the previous one. Already
/// running image decodes cannot be cancelled by Flutter, but stale completions
/// are never counted or allowed to start additional work.
...
'''
    ...
    if binding_test.exists():
        ...
        if "visual-boundary" not in test:
            first = """                assetIdOf: (layer) => layer.assetId,
              );"""
            replacement = """                assetIdOf: (layer) => layer.assetId,
                repaintBoundaryKey: const ValueKey('visual-boundary'),
              );"""
            test = replace_exact(test, first, replacement, "first media-builder test setup")
            ancestor = """              expect(
                find.ancestor(
        ...
        if "disabled-boundary" not in test:
            disabled = """                assetIdOf: (layer) => layer.assetId,
            ...
            test = replace_exact(test, disabled, replacement, "disabled boundary test setup")
            ancestor = """              expect(
                find.ancestor(
                  of: unavailable,
                  matching: find.byType(RepaintBoundary),
...
Visual assets can now carry positive decode-size hints, applied consistently to display and prefetch providers. A bounded prefetch controller deduplicates and caps each feed window, limits concurrent warmups, reports missing/failing assets, and supersedes queued work when the visible window changes. Active Flutter decodes are allowed to finish safely, but stale generations cannot start successor work or affect current-window accounting.
'''
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        raise SystemExit("usage: renderer_prefetch_patch.py <repository-root>")
