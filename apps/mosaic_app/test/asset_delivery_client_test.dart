...
void main() {
  ...
  test('404 is briefly negative-cached while transient failures are not', () async {
    var requests = 0;
    final client = AssetDeliveryClient(
      baseUri: Uri.parse('https://api.example.test/'),
      client: MockClient((request) async {
  ...
  test('managed adapters validate identity, kind and same-origin HTTPS URLs', () async {
    ...
    final visual = await ManagedVisualAssetResolver(client).resolve('image_a');
    expect(visual, isNotNull);
    final visualSource = visual!.source as NetworkPlayVisualSource;
    expect(visualSource.uri.toString(), 'https://api.example.test/v1/assets/image_a/content/primary');
    ...
    final poster = await ManagedVideoPosterResolver(client).resolvePoster('video_a');
    expect(poster?.id, 'video_a:poster');

    final audio = await ManagedAudioAssetResolver(client).resolve('audio_a');
    expect(audio?.uri.toString(), 'https://api.example.test/v1/assets/audio_a/content/primary');
  ...
  test('canvas resolution is bounded, identity-checked and works on explicit localhost', () async {
    var requests = 0;
    final client = AssetDeliveryClient(
    ...
    expect((await resolver.resolve('canvas_a'))?.id, 'canvas_a');
    expect(requests, 1);
    expect(await ManagedVisualAssetResolver(client).resolve('image_a'), isNull);
  });
}
