...
class MetricsRepository implements MosaicRepository {
  ...
  async registerActorAccess(): Promise<ActorAccessRegistration> { return 'created'; }
...
test('metrics expose bounded HTTP labels and active release information', async () => {
  ...
  assert.match(response.body, /mixli_http_request_duration_seconds_count\{method="POST",route="\/v1\/actors\/:actorId\/bind-user",status_class="4xx"\} 1/);
