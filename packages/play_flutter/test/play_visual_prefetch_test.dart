...
final class _Resolver implements PlayVisualAssetResolver {
  _Resolver({required this.assets, this.failures = const <String>{}});
  ...
  Future<PlayVisualAsset?> resolve(String assetId) {
    ...
    if (failures.contains(assetId)) {
      return Future<PlayVisualAsset?>.error(StateError('failed:$assetId'));
    }
...
void main() {
  test('decode dimensions and operation budgets must be positive', () {
    expect(
      () => PlayVisualAsset(id: 'visual_a', source: _source(), cacheWidth: 0),
      throwsRangeError,
    );
    expect(
      () => PlayVisualAsset(id: 'visual_a', source: _source(), cacheHeight: -1),
      throwsRangeError,
    );
    expect(
  ...
  testWidgets('deduplicates, caps and bounds concurrent operations', (
    ...
    final pending = controller.prefetch(context, const [
      'a',
      'a',
      ' ',
      'b',
  ...
  testWidgets('reports missing and failed assets while continuing the window', (
    ...
    final context = await _pumpContext(tester);
    final resolver = _Resolver(
      assets: {'missing': null, 'good': _asset('good')},
    ...
    final report = await controller.prefetch(context, const [
      'missing',
      'broken',
      'good',
    ]);
