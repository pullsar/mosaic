...
PlayFrameSample _sample({required int totalMs, int? buildMs, int? rasterMs}) =>
    PlayFrameSample(
...
void main() {
  group('PlayFramePerformanceAccumulator', () {
    test('rejects impossible refresh rates and sample windows', () {
      for (final refreshRate in <double>[0, -1, double.infinity, double.nan]) {
        expect(
          () => PlayFramePerformanceAccumulator(targetRefreshHz: refreshRate),
          throwsArgumentError,
        );
      }
      ...
      expect(
        () =>
            PlayFramePerformanceAccumulator(targetRefreshHz: 60, maxSamples: 0),
        throwsArgumentError,
      );
    });

    test('summarizes a 60 Hz profiling window deterministically', () {
      final accumulator = PlayFramePerformanceAccumulator(targetRefreshHz: 60)
    ...
    test('uses the tighter 120 Hz frame budget', () {
      final accumulator = PlayFramePerformanceAccumulator(
        targetRefreshHz: 120,
      )..recordAll(<PlayFrameSample>[_sample(totalMs: 8), _sample(totalMs: 9)]);
    ...
    test('retains only the bounded most-recent sample window', () {
      final accumulator =
          PlayFramePerformanceAccumulator(targetRefreshHz: 60, maxSamples: 3)
            ..recordAll(<PlayFrameSample>[
              _sample(totalMs: 5),
              _sample(totalMs: 10),
              _sample(totalMs: 15),
  ...
  test(
    'constructing a performance probe is inert until explicitly started',
    () {
