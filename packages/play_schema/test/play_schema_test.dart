...
void main() {
  ...
  test('all reference fixtures parse and validate', () {
    ...
    for (final fixture in fixtures) {
      final raw = jsonDecode(File('fixtures/$fixture').readAsStringSync())
      ...
      final errors = validator
          ...
          .toList();
      expect(errors, isEmpty, reason: '$fixture: ${errors.map((e) => e.message)}');
    }
  });

  test('rejects unresolved transitions', () {
    final raw = jsonDecode(File('fixtures/where_is_this.json').readAsStringSync())
        as Map<String, Object?>;
    final states = Map<String, Object?>.from(raw['states']! as Map)
      ..['guess'] = {
        ...Map<String, Object?>.from(
          (raw['states']! as Map)['guess']! as Map,
        ),
  ...
  test('enforces prompt copy budget', () {
    ...
    final presentation = Map<String, Object?>.from(guess['presentation']! as Map);
    final layers = (presentation['layers']! as List)
        .map((layer) => Map<String, Object?>.from(layer as Map))
        .toList();
    layers[1] = {
      ...layers[1],
      'value': 'This prompt deliberately contains far too many words for a consumer Play surface and should never be accepted by publication validation',
    };
    states['guess'] = {
      ...guess,
