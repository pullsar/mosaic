...
MosaicEventEnvelope _event({String actorId = 'actor_1', String eventId = 'evt_1'}) =>
    MosaicEventEnvelope(
      eventId: eventId,
...
void main() {
  test('registers one actor then accepts inserted and duplicate events', () async {
    ...
    expect(requests.where((request) => request.url.path == '/v1/actors'), hasLength(1));
    expect(requests.where((request) => request.url.path == '/v1/events'), hasLength(2));
  });
  ...
  test('classifies retryable and permanent event responses separately', () async {
    var eventStatus = 429;
    final client = MockClient((request) async {
