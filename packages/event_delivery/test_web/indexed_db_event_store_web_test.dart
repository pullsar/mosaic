...
String _databaseName(String suffix) => 'mosaic_event_test_${suffix}_${secureUuidV4()}';

void main() {
  test('actor identity and queued events survive browser-store reopen', () async {
  ...
  test('duplicate enqueue preserves retry identity and backoff state', () async {
  ...
  test('queue pressure evicts oldest low-value event before critical work', () async {
    ...
    try {
      ...
      final ids = (await store.due(now: now.add(const Duration(minutes: 1))))
          .map((queued) => queued.envelope.eventId)
          .toSet();
  ...
  test('critical events are never evicted solely to meet the count cap', () async {
    ...
    try {
      await store.enqueue(_event('critical_a'), priority: EventPriority.critical);
      await store.enqueue(_event('critical_b'), priority: EventPriority.critical);

      final ids = (await store.due(now: DateTime.utc(2026, 8, 29)))
          .map((queued) => queued.envelope.eventId)
          .toSet();
  ...
  test('age pruning drops stale analytics but preserves stale critical work', () async {
    ...
    try {
      ...
      final ids = (await store.due(now: now))
          .map((queued) => queued.envelope.eventId)
          .toSet();
  ...
  test('invalid production outbox policy fails before opening IndexedDB', () async {
    await expectLater(
