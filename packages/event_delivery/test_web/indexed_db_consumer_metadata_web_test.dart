...
void main() {
  test(
    'consumer metadata survives reopen without disturbing identity or outbox',
    () async {
      ...
      try {
        ...
        await store.writeConsumerMetadata(
          'preferences.v1',
          '{"interestTopicIds":[]}',
        );
        ...
        expect(
          await store.readConsumerMetadata('actor_id'),
          'consumer-shadow-value',
        );
        expect(
          (await store.due()).single.envelope.eventId,
          'evt_consumer_metadata',
        );
  ...
  test('consumer metadata keys and values are bounded', () async {
    ...
    try {
      ...
      expect(
        () => store.writeConsumerMetadata(
          'preferences.v1',
          List<String>.filled(256 * 1024 + 1, 'x').join(),
        ),
        throwsArgumentError,
      );
    } finally {
